...
export default function ExamWorkspace() {
  ...
  function openEvidence(reference: EvidenceRef) {
    ...
    if (reference.sourceType === 'drawing') { setDrawingOpen(true); return; }
...
function reviewStatusLabel(status: ReviewStatus) { return ({ ai_proposed: 'AI 제안', reviewing: '검토 중', confirmed: '확정', modified: '수정 확정', rejected: '제외', unsupported: '근거 부족' } satisfies Record<ReviewStatus, string>)[status]; }
function evidenceLevelLabel(level: ReviewItem['evidenceLevel']) { return level === 'explicit' ? '명시적 근거' : level === 'inferred' ? '추론 근거' : '근거 부족'; }
function FlowCard({ title, fallbackText, item, onSelect }: { title: string; fallbackText: string; item: ReviewItem | null; onSelect: (entityId: string) => void }) { return <article className={`ai-proposal ${item?.reviewStatus ?? 'unsupported'}`}><span>{item ? reviewStatusLabel(item.reviewStatus) : '근거 정보 없음'}</span><h3>{title}</h3><p>{item?.text || fallbackText}</p><button type="button" disabled={!item} onClick={() => item && onSelect(item.entityId)}>{item ? `원문 근거 ${item.sourceRefs.length}개` : '새 AI 분석 필요'}</button></article>; }
