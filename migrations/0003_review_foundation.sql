...
CREATE INDEX IF NOT EXISTS idx_claim_versions_case_current
  ON claim_versions(user_id, application_number, is_current, updated_at DESC);
...
CREATE TABLE IF NOT EXISTS examination_rounds (
  ...
  notice_document_number TEXT NOT NULL,
  notice_date TEXT NOT NULL,
  documents_json TEXT NOT NULL,
  connection_status TEXT NOT NULL CHECK (connection_status IN ('linked', 'needs_confirmation')),
  source_hash TEXT NOT NULL,
  created_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at TEXT NOT NULL DEFAULT CURRENT_TIMESTAMP,
...
CREATE INDEX IF NOT EXISTS idx_examination_rounds_case_number
  ON examination_rounds(user_id, application_number, round_number);
...
CREATE INDEX IF NOT EXISTS idx_issues_case_status
  ON issues(user_id, application_number, review_status, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_claim_features_case_claim
  ON claim_features(user_id, application_number, claim_number, review_status);
...
CREATE INDEX IF NOT EXISTS idx_evidence_refs_entity
  ON evidence_refs(user_id, application_number, entity_type, entity_id, position);
...
CREATE INDEX IF NOT EXISTS idx_review_decisions_case_status
  ON review_decisions(user_id, application_number, status, updated_at DESC);
...
CREATE INDEX IF NOT EXISTS idx_search_strategy_versions_case
  ON search_strategy_versions(user_id, application_number, version_number DESC);
...
CREATE INDEX IF NOT EXISTS idx_candidate_documents_case_status
  ON candidate_documents(user_id, application_number, review_status, updated_at DESC);
